...
import { AtomicalRevealBuilder } from "../utils/atomical-reveal-builder";
const tinysecp: TinySecp256k1Interface = require('tiny-secp256k1');
initEccLib(tinysecp as any);
const ECPair: ECPairAPI = ECPairFactory(tinysecp);
export class GenerateDftRevealTxInteractiveCommand implements CommandInterface {
  constructor(
    private electrumApi: ElectrumApiInterface,
    private options: BaseRequestOptions,
    private address: string,
    private ticker: string,
    private finnalCopiedData: string,
  ...
  async run(): Promise<any> {
    ...
    const atomicalBuilder = new AtomicalRevealBuilder({
      ...
      finalCopiedData: this.finnalCopiedData,
    });
